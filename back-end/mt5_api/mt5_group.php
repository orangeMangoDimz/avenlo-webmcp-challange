   ...
   private function OldMarginRateInitialSet(&$symbol, $obj)
     {
      ...
      if(is_numeric($symbol->MarginLong) && (float)$symbol->MarginLong!=0.0)
        {
         $buyLimit      = $symbol->MarginRateInitial[MTEnMarginRateTypes::MARGIN_RATE_BUY_LIMIT];
         $buyStop       = $symbol->MarginRateInitial[MTEnMarginRateTypes::MARGIN_RATE_BUY_STOP];
         $buyStopLimit  = $symbol->MarginRateInitial[MTEnMarginRateTypes::MARGIN_RATE_BUY_STOP_LIMIT];
         $marginLongVal = (float)$symbol->MarginLong;
         $marginLimitLong     = is_numeric($buyLimit) ? ((float)$buyLimit / $marginLongVal) : 0;
         $marginStopLong      = is_numeric($buyStop) ? ((float)$buyStop / $marginLongVal) : 0;
         $marginStopLimitLong = is_numeric($buyStopLimit) ? ((float)$buyStopLimit / $marginLongVal) : 0;
        }

      if(is_numeric($symbol->MarginShort) && (float)$symbol->MarginShort!=0.0)
        {
         $sellLimit      = $symbol->MarginRateInitial[MTEnMarginRateTypes::MARGIN_RATE_SELL_LIMIT];
         $sellStop       = $symbol->MarginRateInitial[MTEnMarginRateTypes::MARGIN_RATE_SELL_STOP];
         $sellStopLimit  = $symbol->MarginRateInitial[MTEnMarginRateTypes::MARGIN_RATE_SELL_STOP_LIMIT];
         $marginShortVal = (float)$symbol->MarginShort;
         $marginLimitShort     = is_numeric($sellLimit) ? ((float)$sellLimit / $marginShortVal) : 0;
         $marginStopShort      = is_numeric($sellStop) ? ((float)$sellStop / $marginShortVal) : 0;
         $marginStopLimitShort = is_numeric($sellStopLimit) ? ((float)$sellStopLimit / $marginShortVal) : 0;
        }
