<template>
  <div class="ir-list-container">
    ...
    <div class="ir-list-page-header">
      <div class="ir-list-page-title">
        <h1 class="ir-list-page-title__heading">
          {{ t("page_ibRiskReview_title") }}
        </h1>
        <p class="ir-list-page-title__desc">{{ t("page_ibRiskReview_sub") }}</p>
      </div>
          ...
          <tbody>
            <tr v-for="ib in list" :key="ib.id" class="ir-list-table__row">
              ...
              <td class="ir-list-table__cell ir-list-table__cell--sticky">
                <button
                  v-if="hasRiskReviewPermission"
                  type="button"
                  class="ir-list-btn-action ir-list-btn-action--detail"
                  @click="onRiskReview(ib)"
                >
                  <i class="fas fa-shield-alt"></i>
                  {{ t("ibRr_btn_riskReview") }}
                </button>
                <button
    ...
    <RiskReviewModal
      v-if="riskReviewRow"
      :row="riskReviewRow"
      @close="riskReviewRow = null"
      @success="handleRiskReviewSuccess"
    />
  </div>
</template>
...
import RiskReviewModal from "@/components/ib/RiskReviewModal.vue";
...
const hasRiskReviewPermission = computed(() =>
  authStore.hasPermission("page_ib_risk_review_risk_review"),
);
const hasRejectPermission = computed(() =>
  authStore.hasPermission("page_ib_risk_review_reject"),
);

const router = useRouter();
const riskReviewRow = ref(null);
...
const loadList = async () => {
  try {
    ...
    const response = await ibPartnersApi.getRiskReviewList(params);
    ...
    console.error("Failed to load risk review list:", error);
    list.value = [];
...
/** Risk Review：打开弹窗 */
const onRiskReview = (ib) => {
  riskReviewRow.value = ib;
};

const handleRiskReviewSuccess = () => {
  riskReviewRow.value = null;
  loadList();
  alert(t("ibRr_alert_reviewSubmitted"));
};

/** Reject：回退到 pending_initial_review，显示接口返回信息 */
const onReject = async (ib) => {
  ...
  if (
    !confirm(
      tParams(
        "ibRr_confirm_reject",
        'Reject IB "{name}"? Status will revert to pending initial review.',
        { name },
      ),
    )
  ) {
  ...
  try {
    const response = await ibPartnersApi.rejectIbPartner(ib.id, {
      logSubModuleKey: "ib_risk",
    });
    const message =
      response && response.message
        ? response.message
        : t("ibRr_alert_rejectOk");
    alert(message);
    loadList();
  } catch (err) {
    const raw = err.response?.data?.message || err.message || "";
    alert(raw || t("ibRr_alert_rejectFailed"));
  }
};
