    ...
    <header class="operations-page-header">
      ...
      <div class="operations-header-actions">
        <div class="runtime-compact" :class="statusClass">
          ...
          <label class="runtime-switch" :aria-label="toggleLabel">
            <input :checked="enabled" type="checkbox" :aria-label="toggleLabel" @change="handleToggle" />
            <span></span>
          </label>
        </div>
        <PageHeaderActions />
...
import { isWebMcpEnabled, setWebMcpEnabled, subscribeWebMcpEnabled } from "@/services/adminWebMcpSettings";
...
const toggleLabel = computed(() => `${enabled.value ? "Disable" : "Enable"} WebMCP browser tools`);
...
const handleToggle = (event) => { enabled.value = setWebMcpEnabled(event.target.checked); };
...
onMounted(() => {
  ...
  unsubscribeWebMcpSetting = subscribeWebMcpEnabled((value) => { enabled.value = value; });
  recentActivity.value = activityStore.value.list();
  freshnessTimer = window.setInterval(() => { nowTick.value = Date.now(); }, 60000);
  loadOverview();
});
onUnmounted(() => { requestSequence++; closeInvestigation(); document.removeEventListener("click", handleExportMenuOutside); unsubscribeWebMcpSetting(); if (freshnessTimer !== null) window.clearInterval(freshnessTimer); });
</script>
