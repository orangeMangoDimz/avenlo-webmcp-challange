<template>
  <!-- 通用 settings-card 容器，跟 KYCSettings.vue 里其它卡片视觉一致 -->
  <div v-if="loading" class="ekg-loading-container">
  ...
  <div v-else class="settings-card">
    <div class="card-header" @click="cardCollapsed = !cardCollapsed">
      <div class="card-header-content">
        <h2><i class="fas fa-plug"></i> {{ t("externalKyc_card_title") }}</h2>
                    ...
                    >
                      <div class="gateway-detail-section">
                        ...
                        <div
                          ...
                          class="ekg-loading-container compact"
                        >
                          <i class="fas fa-spinner fa-spin"></i>
                        </div>
                        <p
                        ...
                        <div v-else class="template-list">
                          <div
                            v-for="tpl in getTemplatesState(gateway.id).items"
                            :key="tpl.id"
                            class="template-item"
      ...
      <div class="modal-body">
        ...
        <div class="form-group">
          ...
          <select v-model="form.environment" class="form-input">
            <option value="sandbox">{{ t("externalKyc_env_sandbox") }}</option>
        ...
        <div class="form-group">
          <label class="form-label">{{ t("externalKyc_label_baseUrl") }}</label>
          <input
            type="text"
            class="form-input"
        ...
        <div class="form-group">
          ...
          <input type="text" class="form-input" v-model="form.iframeBaseUrl" />
        </div>
        <div class="form-group">
          <label class="form-label">{{
...
<style scoped>
/* 跟原 ExternalKycSettings.vue 一致，仅删掉外层 page-header / 全局 layout 相关样式 */
.ekg-loading-container {
  text-align: center;
  padding: 60px 20px;
...
.ekg-loading-container i {
  font-size: 48px;
  color: var(--color-brand);
  margin-bottom: 20px;
}

.ekg-loading-container.compact {
  padding: 24px;
  background: transparent;
  box-shadow: none;
}

.ekg-loading-container.compact i {
  font-size: 22px;
  margin-bottom: 0;
}

.ekg-loading-container p {
  font-size: 16px;
