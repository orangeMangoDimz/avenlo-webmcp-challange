    ...
    <div class="modal" style="max-width: 600px" @click.stop>
      <div class="modal-header">
        <h2 class="modal-title">
          ...
          {{
            rule ? t("kycTplRModal_title_edit") : t("kycTplRModal_title_create")
          }}
        </h2>
        ...
        <form @submit.prevent="handleSubmit">
          <div class="form-group">
            <label class="form-label">
              <i class="fas fa-tag"></i> {{ t("kycTplRModal_label_ruleType") }}
            </label>
            <select class="form-select" v-model="formData.ruleType" required>
              <option value="">{{ t("kycTplRModal_select_ruleType") }}</option>
              <option value="jump_to">
                {{ t("kycTplRModal_opt_jumpTo") }}
              </option>
              <option value="reject">{{ t("kycTplRModal_opt_reject") }}</option>
            </select>
            <small
              style="
          ...
          <div class="form-group">
            <label class="form-label">
              ...
              {{ t("kycTplRModal_label_ruleName") }}
            </label>
            <input
              type="text"
              class="form-input"
              v-model="formData.ruleName"
              :placeholder="t('kycTplRModal_placeholder_ruleName')"
              required
            />
          </div>
          ...
          >
            ...
            >
              ...
              >
              {{ t("kycTplRModal_if_heading") }}
            </h4>

            <div class="form-group" style="margin-bottom: 15px">
              <label class="form-label">
                <i class="fas fa-question-circle"></i>
                {{ t("kycTplRModal_label_triggerQ") }}
              </label>
              <select
                class="form-select"
              ...
              >
                <option value="">
                  {{ t("kycTplRModal_select_question") }}
                </option>
            ...
            <div class="form-group" style="margin-bottom: 0">
              <label class="form-label">
                ...
                {{ t("kycTplRModal_label_selectedAnswer") }}
              </label>
              <select
                class="form-select"
                ...
                <option value="">
                  {{
                    formData.triggerQuestion
                      ? t("kycTplRModal_select_answerOption")
                      : t("kycTplRModal_select_answerFirst")
                  }}
                </option>
                <option
              ...
              <small
                ...
                "
              >
                {{ t("kycTplRModal_hint_triggerAnswer") }}
              </small>
            </div>
          </div>
          ...
          >
            ...
            >
              <label class="form-label">
                ...
                {{ t("kycTplRModal_label_jumpToQ") }}
              </label>
              <select class="form-select" v-model="formData.jumpToQuestion">
                <option value="">
                  {{ t("kycTplRModal_select_jumpTarget") }}
                </option>
                <option v-for="q in questions" :key="q.id" :value="q.id">
                  {{ questionLine(q) }}
                </option>
            ...
            >
              <label class="form-label">
                ...
                {{ t("kycTplRModal_label_rejectMsg") }}
              </label>
              <textarea
                class="form-textarea"
                v-model="formData.rejectMessage"
                :placeholder="t('kycTplRModal_placeholder_rejectMsg')"
                rows="3"
              ></textarea>
              <small
                style="
                ...
                "
              >
                {{ t("kycTplRModal_hint_rejectShown") }}
              </small>
            </div>
          </div>
          ...
          <div class="form-group">
            <div class="form-checkbox">
              ...
              <label for="ruleActive">
                <strong>{{ t("kycTplRModal_label_ruleActive") }}</strong> —
                {{ t("kycTplRModal_label_ruleActiveHint") }}
              </label>
            </div>
          </div>
        </form>
      ...
      <div class="modal-footer">
        <button class="btn btn-secondary" @click="$emit('close')">
          <i class="fas fa-times"></i> {{ t("common_cancel") }}
        </button>
        <button class="btn btn-primary" @click="handleSubmit">
          <i class="fas fa-save"></i>
          {{ rule ? t("kycTplRModal_btn_save") : t("kycTplRModal_btn_create") }}
        </button>
      </div>
    </div>
  </div>
...
const questionLine = (q) =>
  tParams("kycTplRModal_questionLine", "Question #{order} — {text}", {
    order: q.displayOrder,
    text: q.questionText,
  });

const ruleTypeHelp = computed(() => {
  if (formData.value.ruleType === "jump_to")
    return t("kycTplRModal_help_jumpTo");
  if (formData.value.ruleType === "reject")
    return t("kycTplRModal_help_reject");
  return t("kycTplRModal_help_selectType");
});
...
const selectedQuestionOptions = computed(() => {
  ...
  if (question?.questionType === "yes_no") {
    return [
      { id: "yes", optionValue: "Yes", label: t("common_yes") },
      { id: "no", optionValue: "No", label: t("common_no") },
    ];
  }

  return (question?.options || []).map((opt) => ({
    ...opt,
    label: typeof opt === "object" ? opt.optionValue : opt,
  }));
});
...
const handleSubmit = async () => {
  if (
    ...
    alert(t("kycTplRModal_alert_required"));
    return;
  }

  if (formData.value.ruleType === "jump_to" && !formData.value.jumpToQuestion) {
    alert(t("kycTplRModal_alert_needTarget"));
    return;
  }
  ...
  if (
    ...
    alert(t("kycTplRModal_alert_needRejectMsg"));
    return;
  }
  ...
  try {
    ...
    if (props.rule) {
      // 编辑模式
      response = await kycRuleService.updateRule(props.rule.id, payload);
      if (response.success) {
        alert(t("kycTplRModal_alert_updateOk"));
        emit("save");
      } else {
        alert(
          tParams(
            "kycTplRModal_alert_updateFailed",
      ...
      // 新增模式
      response = await kycRuleService.createRule(payload);
      if (response.success) {
        alert(t("kycTplRModal_alert_createOk"));
        emit("save");
      } else {
        alert(
          tParams(
            "kycTplRModal_alert_createFailed",
    ...
    alert(t("kycTplRModal_alert_err"));
  }
};
</script>
