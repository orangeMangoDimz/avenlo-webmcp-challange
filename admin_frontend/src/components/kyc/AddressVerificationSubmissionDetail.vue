<template>
  <div class="kyc-submission-detail">
      ...
      <div class="status-message pending-message">
        <div class="status-message-content">
          ...
          <p class="status-message-title">
            {{ t("addrVerif_detail_assignFirstTitle") }}
          </p>
          <p class="status-message-description">
            {{ t("addrVerif_detail_assignFirstDesc") }}
          </p>
        </div>
      </div>
    </template>
    <!-- Under Review状态：检查审核员是否匹配当前用户 -->
    <template v-else-if="submission.submissionStatus === 'under_review'">
      <div
      ...
      >
        <div class="status-message-content">
          ...
          <p class="status-message-title">
            {{ t("addrVerif_detail_underReviewTitle") }}
          </p>
          <p class="status-message-description">
            {{
              tParams(
                "addrVerif_detail_underReviewDesc",
      ...
      <template v-else>
            ...
            <h3>
              <div class="section-header">
                ...
                >
                  ...
                  {{
                    approvedSections.includes(category.id)
                      ? t("addrVerif_detail_approved")
                      : t("addrVerif_detail_approve")
                  }}
                </button>
              </div>
            </h3>
        ...
        >
          <div class="resubmit-answers-header">
            ...
            <h3>{{ t("addrVerif_detail_additionalInfoTitle") }}</h3>
          </div>
          <div class="resubmit-answers-content">
            ...
            <div v-if="resubmitRequest.additionalNotes" class="resubmit-notes">
              <div class="resubmit-notes-label">
                ...
                {{ t("addrVerif_detail_additionalInstructionsShort") }}
              </div>
        ...
        <div class="kyc-actions-section">
          <div class="kyc-actions-header">
            ...
            <h3>{{ t("addrVerif_detail_finalDecision") }}</h3>
          </div>
          ...
          >
            <div class="approval-progress-header">
              <span class="approval-progress-label">
                ...
                {{ t("addrVerif_detail_sectionProgress") }}
              </span>
              <span
              ...
              >
                {{
                  tParams(
                    "addrVerif_detail_sectionsApproved",
            ...
            <div class="approval-progress-bar">
              <div
                ...
                :style="{
                  width:
                    (approvedSections.length / categorizedAnswers.length) *
                      100 +
                    '%',
                }"
              ></div>
          ...
          <div class="kyc-actions-buttons">
            ...
            >
              <i class="fas fa-file-upload"></i>
              {{ t("addrVerif_detail_needMoreDocs") }}
            </button>
            <button
            ...
            >
              <i class="fas fa-times"></i> {{ t("addrVerif_detail_reject") }}
            </button>
          </div>
        </div>

        <div class="need-docs-section" :class="{ show: showNeedDocsSection }">
          <div class="need-docs-header">
            <h4>
              <i class="fas fa-file-upload"></i>
              {{ t("addrVerif_detail_requestMoreTitle") }}
            </h4>
          </div>
          <div class="need-docs-content">
            ...
            <div class="need-docs-items-section">
              <div class="need-docs-items-header">
                <h5>
                  ...
                  {{ t("addrVerif_detail_requestItemsTitle") }}
                </h5>
                <div class="add-item-buttons">
                  ...
                  >
                    ...
                    {{ t("addrVerif_detail_addQuestion") }}
                  </button>
                  <button
                  ...
                  >
                    <i class="fas fa-plus"></i>
                    {{ t("addrVerif_detail_addDocument") }}
                  </button>
                </div>
              </div>
              <div class="items-list">
                ...
                >
                  ...
                  {{ t("addrVerif_detail_emptyItemsHint") }}
                </div>
                <!-- Question Item -->
                <div
                ...
                >
                  ...
                  <div class="item-actions">
                    <button
                      ...
                      :title="t('addrVerif_detail_title_edit')"
                    >
                    ...
                    <button
                      ...
                      :title="t('addrVerif_detail_title_remove')"
                    >
            ...
            <div class="need-docs-notes-field">
              <label for="need-docs-notes"
                ><i class="fas fa-sticky-note"></i>
                {{ t("addrVerif_detail_additionalInstructions") }}</label
              >
              <textarea
                id="need-docs-notes"
                v-model="needDocsNotes"
                :placeholder="t('addrVerif_detail_notesPh')"
              ></textarea>
            </div>
          </div>
          ...
          <div class="need-docs-summary">
            <h5>
              <i class="fas fa-clipboard-list"></i>
              {{ t("addrVerif_detail_requestSummary") }}
            </h5>
            <div class="need-docs-summary-items">
              <span
                v-if="selectedItems.length === 0"
                class="need-docs-summary-empty"
                >{{ t("addrVerif_detail_noItemsSelected") }}</span
              >
              <span
          ...
          <div class="need-docs-actions">
            <button class="btn-cancel-docs" @click="cancelNeedDocs">
              <i class="fas fa-times"></i> {{ t("addrVerif_detail_cancel") }}
            </button>
            <button class="btn-send-request" @click="sendDocumentRequest">
              <i class="fas fa-paper-plane"></i>
              {{ t("addrVerif_detail_sendRequest") }}
            </button>
          </div>
        </div>
      </template>
    ...
    <!-- Approved状态：显示批准信息 -->
    <div
      v-else-if="submission.submissionStatus === 'approved'"
      class="status-message approved-message"
    >
      <div class="status-message-content">
        <i class="fas fa-check-circle"></i>
        <p class="status-message-title">
          {{ t("addrVerif_detail_approvedTitle") }}
        </p>
        <p class="status-message-description">
          {{
            tParams(
              "addrVerif_detail_approvedDesc",
    ...
    >
      <div class="status-message-content">
        ...
        <p class="status-message-title">
          {{ t("addrVerif_detail_rejectedTitle") }}
        </p>
        <p class="status-message-description">
          {{
            tParams(
              "addrVerif_detail_rejectedDesc",
          ...
          >
            <br /><strong>{{ t("addrVerif_detail_reasonLabel") }}</strong>
            {{
              submission.rejectionReason ||
              (submissionDetails && submissionDetails.rejectionReason) ||
              t("addrVerif_na")
    ...
    >
      <div class="status-message-content">
        <i class="fas fa-file-upload"></i>
        <p class="status-message-title">
          {{ t("addrVerif_detail_resubmitTitle") }}
        </p>
        <p class="status-message-description">
          {{ t("addrVerif_detail_resubmitDesc") }}
        </p>
      </div>
    </div>
      ...
      <div class="add-item-modal-content">
        <div class="add-item-modal-header">
          <h3>
            ...
            {{
              isEditMode
                ? addItemType === "question"
                  ? t("addrVerif_detail_editQuestion")
                  : t("addrVerif_detail_editDocument")
                : addItemType === "question"
                  ? t("addrVerif_detail_addQuestionModal")
                  : t("addrVerif_detail_addDocumentModal")
            }}
          </h3>
          ...
          <form @submit.prevent="saveItemToList">
            ...
            <div class="add-item-field" v-if="addItemType === 'question'">
              <label for="itemQuestionText"
                >{{ t("addrVerif_detail_label_questionText") }}
                <span style="color: var(--color-danger)">*</span></label
              >
              <textarea
                id="itemQuestionText"
                v-model="itemForm.questionText"
                :placeholder="t('addrVerif_detail_ph_questionText')"
                rows="3"
                required
              ></textarea>
            </div>
            ...
            <div class="add-item-field" v-if="addItemType === 'document'">
              <label for="itemDocumentName"
                >{{ t("addrVerif_detail_label_documentName") }}
                <span style="color: var(--color-danger)">*</span></label
              >
              <input
                type="text"
                ...
                :placeholder="t('addrVerif_detail_ph_documentName')"
                required
              />
            </div>
            ...
            <div class="add-item-field" v-if="addItemType === 'question'">
              <label for="itemQuestionType"
                >{{ t("addrVerif_detail_label_questionType") }}
                <span style="color: var(--color-danger)">*</span></label
              >
              <select
                id="itemQuestionType"
              ...
              >
                <option value="">
                  {{ t("addrVerif_detail_selectQuestionType") }}
                </option>
                <option
            ...
            <div class="add-item-field" v-if="addItemType === 'document'">
              <label for="itemDocumentType">{{
                t("addrVerif_detail_label_documentType")
              }}</label>
              <select id="itemDocumentType" v-model="itemForm.documentType">
                <option value="ID_CARD">
                  {{ t("addrVerif_detail_doc_identityCard") }}
                </option>
                <option value="PASSPORT">
                  {{ t("addrVerif_detail_doc_passport") }}
                </option>
                <option value="DRIVERS_LICENSE">
                  {{ t("addrVerif_detail_doc_driversLicense") }}
                </option>
                <option value="PROOF_ADDRESS">
                  {{ t("addrVerif_detail_doc_proofOfAddress") }}
                </option>
                <option value="BANK_STATEMENT">
                  {{ t("addrVerif_detail_doc_bankStatement") }}
                </option>
                <option value="UTILITY_BILL">
                  {{ t("addrVerif_detail_doc_utilityBill") }}
                </option>
                <option value="INCOME_PROOF">
                  {{ t("addrVerif_detail_doc_incomeProof") }}
                </option>
                <option value="TAX_DOCUMENT">
                  {{ t("addrVerif_detail_doc_taxDocument") }}
                </option>
                <option value="EMPLOYMENT_LETTER">
                  {{ t("addrVerif_detail_doc_employmentLetter") }}
                </option>
                <option value="OTHER">
                  {{ t("addrVerif_detail_doc_other") }}
                </option>
              </select>
            </div>
            ...
            <div class="add-item-field">
              <label for="itemHelpText">{{
                t("addrVerif_detail_label_helpText")
              }}</label>
              <input
                type="text"
                id="itemHelpText"
                v-model="itemForm.helpText"
                :placeholder="t('addrVerif_detail_ph_helpText')"
              />
            </div>
            ...
            <div class="add-item-field" v-if="addItemType === 'question'">
              <label for="itemValidation">{{
                t("addrVerif_detail_label_validation")
              }}</label>
              <input
                type="text"
                id="itemValidation"
                v-model="itemForm.validation"
                :placeholder="t('addrVerif_detail_ph_validation')"
              />
            </div>
            ...
            >
              <label>{{ t("addrVerif_detail_label_answerOptions") }}</label>
              <div class="answer-options-container">
                ...
                >
                  <input
                    ...
                    :placeholder="
                      tParams('addrVerif_detail_ph_optionN', 'Option {n}', {
                        n: optIndex + 1,
                      })
                    "
            ...
            >
              <label for="itemFileDocumentTypes">
                ...
                {{ t("addrVerif_detail_label_acceptedDocTypes") }}
              ...
              >
                ...
                {{ t("addrVerif_detail_hint_multiSelect") }}
              </small>
            </div>
            ...
            <div class="add-item-field" style="grid-column: 1 / -1">
              ...
              >
                <input
                  ...
                  "
                />
                <span>{{ t("addrVerif_detail_requiredField") }}</span>
              </label>
            </div>
          </form>
        </div>
        <div class="add-item-modal-footer">
          ...
          <button class="btn-modal-save" @click="saveItemToList">
            ...
            {{
              isEditMode
                ? t("addrVerif_detail_updateItem")
                : t("addrVerif_detail_addItem")
            }}
          </button>
        </div>
      </div>
    ...
    >
      <div class="reject-modal-content">
        <div class="reject-modal-header">
          <h3>
            <i class="fas fa-times-circle"></i>
            {{ t("addrVerif_detail_rejectModalTitle") }}
          </h3>
        ...
        <div class="reject-modal-body">
          <div class="reject-field">
            <label for="rejection-reason"
              >{{ t("addrVerif_detail_rejectReasonLabel") }}
              <span style="color: var(--color-danger)">*</span></label
            >
            <textarea
              id="rejection-reason"
              v-model="rejectionReason"
              :placeholder="t('addrVerif_detail_rejectReasonPh')"
              rows="5"
              required
            ></textarea>
          </div>
          <div class="reject-warning">
            <i class="fas fa-exclamation-triangle"></i>
            <span>{{ t("addrVerif_detail_rejectWarning") }}</span>
          </div>
        </div>
        <div class="reject-modal-footer">
          ...
          >
            <i class="fas fa-times"></i>
            {{ t("addrVerif_detail_confirmRejection") }}
          </button>
        </div>
      </div>
    </div>
...
export default {
  name: "AddressVerificationSubmissionDetail",
  props: {
    submission: {
      type: Object,
      required: true,
  ...
  setup(props, { emit }) {
    ...
    // Question Types - 从 KYC Template 页面获取
    ...
    const getAssignedReviewerId = () => {
      return (
        submissionDetails.value?.reviewedBy ??
        props.submission.reviewedBy ??
        null
      );
    };

    const reviewerDisplayName = computed(() => {
      if (submissionDetails.value?.reviewerName) {
        return submissionDetails.value.reviewerName;
      }

      const assignedActivity = (submissionDetails.value?.activities || []).find(
        (activity) =>
          activity.activityType === "assigned" && activity.performedByName,
      );

      return (
        assignedActivity?.performedByName || t("addrVerif_reviewerAnonymous")
      );
    });
    ...
    const isCurrentUserReviewer = computed(() => {
      ...
      const assignedReviewerId = getAssignedReviewerId();

      if (!currentUserId || !assignedReviewerId) {
        return false;
      }

      return String(currentUserId) === String(assignedReviewerId);
    });
    ...
    const categorizedAnswers = computed(() => {
      ...
      if (Array.isArray(submissionDetails.value.answers)) {
        return submissionDetails.value.answers.map((category, index) => {
          ...
          const sourceAnswers = category.answers || [];

          // 转换为组件内部统一格式，并添加 questionType
          const answers = sourceAnswers.map((question) => {
            let questionType = question.questionType;

            const rawValue = question.value;
            if (!questionType && rawValue) {
            ...
            return {
              ...
              files:
                ...
                (Array.isArray(question.value) ? question.value : null),
            };
          });
    ...
    const getFileUrl = (file) => {
      ...
      if (typeof file === "object") {
        return file.filePath || file.s3Url || file.url || "";
      }
    ...
    const getFileName = (file) => {
      ...
      if (typeof file === "object") {
        return basename(
          file.filePath || file.downloadUrl || file.s3Url || file.url || "",
        );
      }
      if (typeof file === "string") {
    ...
    const loadSubmissionDetails = async () => {
      ...
      // approved状态不加载详情
      if (props.submission.submissionStatus === "approved") {
        return;
      }
      ...
      try {
        ...
        // 如果是 under_review 状态且是当前审批员，加载补充问题和文件的详情
        if (
          props.submission.submissionStatus === "under_review" &&
          isCurrentUserReviewer.value
        ) {
          try {
            const resubmitResponse =
              await addressVerificationSubmissionService.getResubmitAnswers(
                props.submission.submissionId,
              );
    ...
    const confirmReject = () => {
      if (!rejectionReason.value.trim()) {
        alert(t("addrVerif_alert_rejectReason"));
        return;
      }
      emit(
        "reject",
    ...
    const saveItemToList = () => {
      if (addItemType.value === "question") {
        if (!itemForm.value.questionText.trim()) {
          alert(t("addrVerif_alert_enterQuestionText"));
          return;
        }
        if (!itemForm.value.questionType) {
          alert(t("addrVerif_alert_selectQuestionType"));
          return;
        }
        ...
        if (
          ...
          if (validOptions.length === 0) {
            alert(t("addrVerif_alert_choiceOptions"));
            return;
          }
        }
        ...
        if (itemForm.value.questionType === "file_upload") {
          if (itemForm.value.fileDocumentTypes.length === 0) {
            alert(t("addrVerif_alert_fileDocTypes"));
            return;
          }
        }
        ...
        if (!itemForm.value.documentName.trim()) {
          alert(t("addrVerif_alert_enterDocumentName"));
          return;
        }
    ...
    const sendDocumentRequest = () => {
      ...
      if (selectedItemsList.length === 0 && !needDocsNotes.value.trim()) {
        alert(t("addrVerif_alert_selectOrNotes"));
        return;
      }
    ...
    onMounted(() => {
      ...
      if (
        ...
        alert(t("addrVerif_alert_assignFirst"));
      }
      loadSubmissionDetails();
    });
