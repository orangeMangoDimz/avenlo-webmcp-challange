  ...
  >
    <div class="bulk-assignment-modal-content">
      <div class="bulk-assignment-modal-header">
        <h3>
          <i class="fas fa-user-check"></i> {{ t("kycBulkAssign_title") }}
        </h3>
      ...
      <div class="bulk-assignment-modal-body">
        ...
        <div class="selected-kyc-list">
          <h4>
            ...
            {{ t("kycBulkAssign_selectedHeading") }}
          ...
          <div class="bulk-assignment-content">
            <div class="bulk-assignment-field">
              <label for="bulk-reviewer">{{
                t("kycBulkAssign_assignTo")
              }}</label>
              <select id="bulk-reviewer" v-model="selectedReviewer">
                <option value="">
                  {{ t("kycBulkAssign_selectPlaceholder") }}
                </option>
                <option
            ...
            <div class="bulk-assignment-info">
              ...
              <div class="bulk-assignment-info-item">
                <span class="bulk-assignment-info-label">{{
                  t("kycBulkAssign_assignmentDate")
                }}</span>
                <span class="bulk-assignment-info-value">{{
                  formatDate(new Date())
                }}</span>
              ...
              <div class="bulk-assignment-info-item">
                <span class="bulk-assignment-info-label">{{
                  t("kycBulkAssign_assignedBy")
                }}</span>
                <span class="bulk-assignment-info-value">{{
                  currentUser.fullName || t("kycBulkAssign_adminUser")
                }}</span>
              </div>
            </div>
            ...
            <div class="bulk-assignment-field" style="grid-column: 1 / -1">
              <label for="bulk-assignment-notes">{{
                t("kycBulkAssign_notesLabel")
              }}</label>
              <textarea
                id="bulk-assignment-notes"
                v-model="assignmentNotes"
                :placeholder="t('kycBulkAssign_notesPlaceholder')"
              ></textarea>
            </div>
          </div>
        </div>
      ...
      <div class="bulk-assignment-modal-footer">
        ...
        >
          ...
          {{
            loading
              ? t("kycBulkAssign_assigning")
              : t("kycBulkAssign_btnAssign")
          }}
        </button>
      </div>
    </div>
  ...
  setup(props, { emit }) {
    ...
    const loadReviewers = async () => {
      try {
        ...
        const response = await kycReviewerService.getReviewers();
    ...
    const confirmAssignment = async () => {
      if (!selectedReviewer.value) {
        alert(t("kycBulkAssign_alert_selectReviewer"));
        return;
      }
      ...
      try {
        ...
        // 调用API分配审核员
        await kycSubmissionService.bulkAssign(submissionIds, {
