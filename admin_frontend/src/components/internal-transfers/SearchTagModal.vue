    ...
    <div :class="['tag-modal', { show: modelValue }]" @click="close">
      <div class="tag-modal-content" @click.stop>
        <div class="tag-modal-header">
          <h3>
            <i class="fas fa-tag"></i> {{ t("internalXfer_searchTag_title") }}
          </h3>
        ...
        <div class="tag-modal-body">
          <div class="form-group">
            <label for="tag-name">{{
              t("internalXfer_searchTag_labelName")
            }}</label>
            <input
              id="tag-name"
              ref="tagNameInput"
              ...
              :placeholder="t('internalXfer_searchTag_namePlaceholder')"
              @keyup.enter="confirm"
            />
          </div>
          ...
          <div class="form-group">
            <label for="search-keywords">{{
              t("internalXfer_searchTag_labelKeywords")
            }}</label>
            <input
              id="search-keywords"
              type="text"
              v-model="formData.searchKeywords"
              :placeholder="t('internalXfer_searchTag_keywordsPlaceholder')"
              @keyup.enter="confirm"
            />
            <small class="help-text">
              <i class="fas fa-info-circle"></i>
              {{ t("internalXfer_searchTag_keywordsHelp") }}
            </small>
          </div>
        </div>
        ...
        <div class="tag-modal-footer">
          ...
          >
            ...
            {{
              processing
                ? t("internalXfer_searchTag_creating")
                : t("internalXfer_searchTag_btnCreate")
            }}
          </button>
        </div>
      </div>
...
const confirm = () => {
  ...
  if (!tagName) {
    alert(t("internalXfer_searchTag_enterName"));
    return;
  }

  if (!keywords) {
    alert(t("internalXfer_searchTag_enterKeywords"));
    return;
  }
