...
const uploadFile = async (fileData) => {
  ...
  try {
    console.log("Starting resubmit file upload for:", fileData.name);
    const response = await clientKycService.uploadResubmitFile(
      props.submissionId,
      props.itemIndex,
      fileData.file,
    );

    console.log("Resubmit upload response:", response);
    ...
    if (response.success && response.data) {
      ...
      files.value[fileIndex] = {
        ...
        filename: response.data.filename || fileData.name,
        error: null,
      };

      // 确保文件名正确显示
      if (response.data.filename) {
        files.value[fileIndex].name = response.data.filename;
      }

      console.log("Resubmit file uploaded successfully:", {
        originalName: fileData.name,
        serverFilename: response.data.filename,
        url: response.data.url,
        fileObject: files.value[fileIndex],
      });
    } else {
      // 上传失败
      files.value[fileIndex] = {
      ...
      console.error("Resubmit upload failed:", response);
    }
  } catch (err) {
    console.error("Failed to upload resubmit file:", err);
    // 上传异常
    files.value[fileIndex] = {
      ...files.value[fileIndex],
      uploading: false,
...
const emitFilesChanged = () => {
  emit("files-changed", props.itemIndex, files.value);
};
