<template>
  <div class="question-list">
    <div v-if="groupedQuestions.length === 0" class="empty-state">
      ...
      <button class="btn btn-primary" @click="$emit('add-category')">
    ...
    >
      ...
      <div class="category-questions">
        ...
        >
          ...
          <div v-if="question.isRequired" class="required-badge">Required</div>
...
const groupedQuestions = computed(() => {
  return props.categories.map((category) => ({
    ...category,
    questions: props.questions.filter((q) => q.categoryId === category.id),
  }));
});
  ...
  if (confirm("Duplicate this question?")) {
    try {
      ...
      if (response.success) {
        ...
        alert(`Failed to duplicate question: ${response.message}`);
      }
    } catch (error) {
