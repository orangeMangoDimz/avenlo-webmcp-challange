<template>
  <div class="container ui-page">
        ...
        <div class="export-wrap">
          ...
          <div :class="['export-dropdown', { show: showExportDropdown }]">
            ...
            >
              ...
              <span>{{ t("ibStatement_export_csv", "CSV") }}</span>
            </button>
            <button
              type="button"
            ...
            >
              ...
              <span>{{ t("ibStatement_export_excel", "Excel") }}</span>
            </button>
          </div>
        </div>
      </div>
      ...
      <p>
        {{
          t(
            ...
            "Select a date range and apply the filter.",
          )
        }}
      </p>
    </div>
    ...
    <template v-else>
      ...
      <div class="transaction-table-container ui-data-region ui-table-scroll">
        ...
        <div v-if="movementPager.totalPages > 1" class="pagination">
          ...
          <div class="pagination-controls">
            ...
            <button
              type="button"
              class="pagination-btn"
              :disabled="movementPager.currentPage === movementPager.totalPages"
              @click="movementPager.goTo(movementPager.currentPage + 1)"
            >
          ...
          <table class="transaction-table accounts-detail-table">
            <thead>
              <tr>
                ...
                <th class="accounts-detail-col-detail"></th>
              </tr>
            </thead>
            <tbody>
              ...
              <template v-else>
                <template v-for="row in accountPagedRows" :key="row.login">
                  <tr
                    :class="{
                      unfunded: row.unfunded,
                      expanded: expandedAccountLogin === String(row.login),
                    }"
                  >
                    <td>
                      <div class="client-info">
                        ...
                        <div class="client-details">
                          ...
                          <span
                            ...
                            "
                            >{{ t("ibStatement_unfunded", "Unfunded") }}</span
                          >
                        </div>
                      </div>
                    </td>
                    ...
                    <td>
                      <div
                        :class="['amount-display', amountTone(row.deposits)]"
                      >
                    ...
                    <td class="accounts-detail-col-detail">
                      <button
                        type="button"
                        class="btn-action btn-detail"
                        :aria-expanded="
                          expandedAccountLogin === String(row.login)
                        "
                        :title="t('ibStatement_detail_toggle', 'Detail')"
                        @click="toggleAccountDetail(row)"
                      >
                        <i
                          :class="
                            expandedAccountLogin === String(row.login)
                        ...
                        {{
                          expandedAccountLogin === String(row.login)
                            ? t("ibStatement_detail_hide", "Hide")
                            : t("ibStatement_detail_toggle", "Detail")
                        }}
                      </button>
                    </td>
                  </tr>
                  <tr
                    v-if="expandedAccountLogin === String(row.login)"
                    class="account-detail-expand-row"
                  >
                    <td colspan="12" class="account-detail-expand-cell">
                      <IbStatementAccountPositionDetail
                        :login="row.login"
                        :ib-partner-id="selectedIbId"
            ...
            <tfoot v-if="filteredAccounts.length">
              <tr class="total-row">
                ...
                <td class="accounts-detail-col-detail"></td>
              </tr>
            </tfoot>
          </table>
        </div>
        ...
        <table class="transaction-table instruments-detail-table">
          ...
          <tfoot v-if="instrumentItems.length">
            <tr class="total-row">
              ...
              <td>
                <strong
                  >{{ formatNumber(instrumentTotal.shareOfVolume, 1) }}%</strong
                >
              </td>
              <td>
                <div
                ...
                >
                  ...
                  <div v-else class="ib-select-search__list">
                    <div
                      ...
                      :class="{
                        'ib-select-option--selected': opt.value === weekFilter,
                      }"
...
import ibStatementReportService from "@/services/ibStatementReportService";
import ibDashboardService from "@/services/ibDashboardService";
import IbStatementAccountPositionDetail from "@/components/ib/IbStatementAccountPositionDetail.vue";
...
const {
  ...
  getActiveExport: () => ibStatementReportService.getActiveExport(),
  enqueueExport: (params) => ibStatementReportService.exportReport(params),
  getExportStatus: (jobId) => ibStatementReportService.getExportStatus(jobId),
  cancelExport: (jobId) => ibStatementReportService.cancelExport(jobId),
  downloadExport: (jobId) => ibStatementReportService.downloadExport(jobId),
  buildFilename: () =>
...
const expandedAccountLogin = ref("");

const toggleAccountDetail = (row) => {
  const login = String(row?.login ?? "");
  if (!login) return;
  expandedAccountLogin.value =
    expandedAccountLogin.value === login ? "" : login;
};
...
const loadStatement = async () => {
  ...
  error.value = "";
  try {
    const response = await ibStatementReportService.getStatement({
...
.accounts-detail-table thead th {
  position: sticky;
  top: 0;
  z-index: 2;
  background: var(--color-surface-soft);
...
.accounts-detail-table tfoot td {
  position: sticky;
  bottom: 0;
  z-index: 2;
  background: var(--color-surface);
...
.account-detail-expand-row td {
  padding: 0 !important;
  background: var(--color-surface-soft);
}

.account-detail-expand-cell {
  border-bottom: 1px solid var(--color-border);
}
